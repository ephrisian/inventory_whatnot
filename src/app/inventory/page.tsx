...
export default function InventoryPage() {
  ...
  const renderCellContent = (item: Item, columnKey: string) => {
    switch (columnKey) {
      ...
      case 'totalValue':
        return formatCurrency(item.totalValue || 0)
      case 'status':
        return (
                  ...
                  <tr className="font-medium">
                    ...
                    {visibleColumns.map((column) => {
                      if (column.key === 'quantity') {
                        return (
                          <td key={column.key} className="px-6 py-4 text-sm font-bold">
                            {sortedItems.reduce((sum, item) => sum + item.quantity, 0)}
                          </td>
                        )
                      } else if (column.key === 'cost') {
                        return (
                          <td key={column.key} className="px-6 py-4 text-sm font-bold text-orange-600">
